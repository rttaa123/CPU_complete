...
module decode(                      // 译码级 - 负责对指令进行解码，产生控制信号
    ...
    output    [161:0]  ID_EXE_bus,  // ID->EXE总线，传递给执行阶段的所有必要信息 (扩展为162位)
...
//-----{指令译码}begin
    ...
    // 新增浮点指令
    wire inst_FADD_S, inst_FSUB_S, inst_FMUL_S, inst_FDIV_S;
    wire inst_FABS_S, inst_FNEG_S, inst_FSQRT_S, inst_FMOV_S;
    wire inst_CVT_W_S, inst_CVT_S_W, inst_C_EQ_S, inst_C_LT_S;
    
    wire op_zero;  // 操作码全0标志，用于识别R型指令
    wire sa_zero;  // sa域全0标志，用于某些R型指令的识别
    wire op_cop1;  // 操作码为0x11，用于识别浮点指令
    
    assign op_zero = ~(|op);         // 判断op是否全为0，若是则op_zero=1
    assign sa_zero = ~(|sa);         // 判断sa是否全为0，若是则sa_zero=1  
    assign op_cop1 = (op == 6'b010001); // 浮点指令的操作码为0x11
    ...
    // 新增浮点指令识别
    // 浮点指令格式：op=0x11, rs指定具体操作类型, funct进一步细分
    assign inst_FADD_S  = op_cop1 & (rs == 5'b10000) & (funct == 6'b000000); // 单精度浮点加法
    assign inst_FSUB_S  = op_cop1 & (rs == 5'b10000) & (funct == 6'b000001); // 单精度浮点减法
    assign inst_FMUL_S  = op_cop1 & (rs == 5'b10000) & (funct == 6'b000010); // 单精度浮点乘法
    assign inst_FDIV_S  = op_cop1 & (rs == 5'b10000) & (funct == 6'b000011); // 单精度浮点除法
    assign inst_FABS_S  = op_cop1 & (rs == 5'b10000) & (funct == 6'b000101); // 单精度浮点绝对值
    assign inst_FNEG_S  = op_cop1 & (rs == 5'b10000) & (funct == 6'b000111); // 单精度浮点取负
    assign inst_FSQRT_S = op_cop1 & (rs == 5'b10000) & (funct == 6'b000100); // 单精度浮点平方根
    assign inst_FMOV_S  = op_cop1 & (rs == 5'b10000) & (funct == 6'b000110); // 单精度浮点移动
    assign inst_CVT_W_S = op_cop1 & (rs == 5'b10000) & (funct == 6'b100100); // 浮点转整数
    assign inst_CVT_S_W = op_cop1 & (rs == 5'b10100) & (funct == 6'b100000); // 整数转浮点
    assign inst_C_EQ_S  = op_cop1 & (rs == 5'b10000) & (funct == 6'b110010); // 浮点比较相等
    assign inst_C_LT_S  = op_cop1 & (rs == 5'b10000) & (funct == 6'b111100); // 浮点比较小于
    ...
    //fpu操作分类 - 为后续生成FPU控制信号做准备
    wire inst_fp_add, inst_fp_sub, inst_fp_mul, inst_fp_div;
    wire inst_fp_abs, inst_fp_neg, inst_fp_sqrt, inst_fp_mov;
    wire inst_fp_cvt_w, inst_fp_cvt_s, inst_fp_cmp_eq, inst_fp_cmp_lt;
    
    assign inst_fp_add    = inst_FADD_S;     // 浮点加法
    assign inst_fp_sub    = inst_FSUB_S;     // 浮点减法  
    assign inst_fp_mul    = inst_FMUL_S;     // 浮点乘法
    assign inst_fp_div    = inst_FDIV_S;     // 浮点除法
    assign inst_fp_abs    = inst_FABS_S;     // 浮点绝对值
    assign inst_fp_neg    = inst_FNEG_S;     // 浮点取负
    assign inst_fp_sqrt   = inst_FSQRT_S;    // 浮点平方根
    assign inst_fp_mov    = inst_FMOV_S;     // 浮点移动
    assign inst_fp_cvt_w  = inst_CVT_W_S;    // 浮点转整数
    assign inst_fp_cvt_s  = inst_CVT_S_W;    // 整数转浮点
    assign inst_fp_cmp_eq = inst_C_EQ_S;     // 浮点比较相等
    assign inst_fp_cmp_lt = inst_C_LT_S;     // 浮点比较小于
    ...
    assign inst_wdest_rd = inst_ADDU | inst_SUBU | inst_SLT  | inst_SLTU
                         ...
                         | inst_SRAV | inst_SRL  | inst_SRLV        // 原有整数指令
                         | inst_FADD_S | inst_FSUB_S | inst_FMUL_S | inst_FDIV_S
                         | inst_FABS_S | inst_FNEG_S | inst_FSQRT_S | inst_FMOV_S
                         | inst_CVT_W_S | inst_CVT_S_W | inst_C_EQ_S | inst_C_LT_S; // 新增浮点指令
//-----{指令译码}end
...
//-----{ID->EXE总线}begin
    ...
    //FPU两个源操作数和控制信号
    wire [11:0] fpu_control;   // FPU控制信号，12位独热码
    wire [31:0] fpu_operand1;  // FPU操作数1
    wire [31:0] fpu_operand2;  // FPU操作数2
    wire        use_fpu;       // 使用FPU标志
    ...
    // 确定FPU的两个操作数
    assign fpu_operand1 = rs_value;  // 浮点指令的第一个操作数
    assign fpu_operand2 = rt_value;  // 浮点指令的第二个操作数
    
    // FPU控制信号，12位独热码
    assign fpu_control = {inst_fp_add,     // FPU操作码，独热编码
                          inst_fp_sub,
                          inst_fp_mul,
                          inst_fp_div,
                          inst_fp_abs,
    ...
    // 使用FPU标志
    assign use_fpu = inst_fp_add | inst_fp_sub | inst_fp_mul | inst_fp_div |
                     inst_fp_abs | inst_fp_neg | inst_fp_sqrt | inst_fp_mov |
                     inst_fp_cvt_w | inst_fp_cvt_s | inst_fp_cmp_eq | inst_fp_cmp_lt;
    ...
    // 打包所有需要传递给执行阶段的信息 (扩展总线宽度)
    assign ID_EXE_bus = {alu_control,alu_operand1, alu_operand2, //EXE需要使用的ALU信息
                        fpu_control,fpu_operand1, fpu_operand2, use_fpu, //EXE需要使用的FPU信息
