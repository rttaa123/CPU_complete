...
//*************************************************************************
//   > 文件名: exe.v
//   > 描述  :多周期CPU的执行模块 (添加FPU支持)
//   > 作者  : 白星炜、饶甜甜 (修改：添加FPU支持)
//   > 日期  : 2025-06-19
//*************************************************************************
module exe(                         // 执行级 - 负责执行算术逻辑运算和浮点运算
    input              EXE_valid,   // 执行级有效信号，表示当前CPU状态处于执行阶段
    input      [161:0] ID_EXE_bus_r,// ID->EXE总线，包含从译码阶段传递过来的所有必要信息 (扩展为162位)
...
//-----{ID->EXE总线}begin
    ...
    //FPU两个源操作数和控制信号
    wire [11:0] fpu_control;    // FPU控制信号，12位独热码，指示FPU执行哪种运算
    wire [31:0] fpu_operand1;   // FPU第一个操作数
    wire [31:0] fpu_operand2;   // FPU第二个操作数
    ...
    assign {alu_control,  // ALU控制信号 - 12位
            ...
            fpu_control,  // FPU控制信号 - 12位
            fpu_operand1, // FPU操作数1 - 32位
            fpu_operand2, // FPU操作数2 - 32位
            use_fpu,      // 使用FPU标志 - 1位
...
//-----{FPU}begin
    // 实例化FPU模块，执行浮点运算
    wire [31:0] fpu_result;     // FPU计算结果
    
    // 调用FPU模块进行浮点计算
    fpu fpu_module(
        .fpu_control  (fpu_control ),  // I, 12, FPU控制信号，指示要执行的具体浮点运算
        .fpu_src1     (fpu_operand1),  // I, 32, FPU第一个操作数
        .fpu_src2     (fpu_operand2),  // I, 32, FPU第二个操作数
        .fpu_result   (fpu_result  )   // O, 32, FPU计算结果
    );
//-----{FPU}end

//-----{执行结果选择}begin
    // 根据use_fpu标志选择ALU或FPU的结果
...
//-----{EXE->MEM总线}begin
    // 打包所有需要传递给访存阶段的信息
    // 总线内容包括访存控制信号、存储数据、执行结果、写回控制信号和PC值
    assign EXE_MEM_bus = {mem_control,   // 访存控制信号 - 4位
                          store_data,    // 存储数据 - 32位
                          exe_result,    // 执行结果(ALU或FPU) - 32位
                          rf_wen,        // 寄存器写使能 - 1位
                          rf_wdest,      // 目标寄存器地址 - 5位
                          pc};           // 指令PC值 - 32位
//-----{EXE->MEM总线}end
