...
//add,sub,slt,sltu均使用该模块
    ...
    //溢出判断
    wire         overflow_flag;  // 溢出标志
    wire add_overflow = (~alu_src1[31] & ~alu_src2[31] &  add_sub_result[31])
                      | ( alu_src1[31] &  alu_src2[31] & ~add_sub_result[31]);

    wire sub_overflow = (~alu_src1[31] &  alu_src2[31] &  add_sub_result[31])
                      | ( alu_src1[31] & ~alu_src2[31] & ~add_sub_result[31]);


    assign overflow_flag = (alu_add && add_overflow) || (alu_sub && sub_overflow);
